// NewCoin Radar — Service Worker v1.0
const CACHE = 'newcoin-radar-v1';
const ASSETS = [
  './',
  './index.html',
  ...
  e.waitUntil(
    caches.open(CACHE).then(cache => {
      return cache.addAll(ASSETS).catch(() => {
        // Partial failure OK — external fonts may be blocked
...
self.addEventListener('fetch', e => {
  ...
  // Always network for API calls (Anthropic / OpenAI / CMC)
